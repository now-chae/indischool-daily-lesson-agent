...
이 저장소는 **완성된 에이전트를 내려받아 설정하고 사용하는 사람**을 위한 패키지입니다. 이 README 하나만 따라 하면 설치·로그인·시험·예약까지 진행할 수 있습니다.
...
> 아래 순서를 위에서부터 그대로 따라 하세요. 별도의 설치 안내 파일은 필요하지 않습니다.
...
HWPX만 사용해야 하는 것은 아닙니다. 다음 형식을 모두 지원합니다.

- `.hwpx`: 가장 권장합니다. 표가 XML로 저장되어 행·열·셀 병합 구조를 안정적으로 확인할 수 있습니다.
- `.hwp`: 사용할 수 있습니다. 다만 바이너리 형식이라 `pyhwp`의 `hwp5txt`·`hwp5html` 변환 단계를 거치며, 문서 구조에 따라 추가 확인이 필요할 수 있습니다.
- `.jpg`, `.jpeg`, `.png`, `.webp`: 사용할 수 있습니다. 표를 이미지로 읽으므로 OpenAI API 키가 필요합니다.

즉, **HWPX는 필수가 아니라 정확도와 설치 안정성 때문에 권장하는 형식**입니다. 어떤 형식이든 파일명에 주간 날짜를 넣고 로컬 주안 폴더에 보관하세요.

HWPX는 표의 행·열을 보존해 판독하고, HWP는 설치 과정에서 `pyhwp`를 함께 설치합니다. OpenAI API 키는 이미지 판독을 선택할 때만 필요합니다.
...
예약 지시문은 다음처럼 입력하면 됩니다.

```text
이 저장소의 인디스쿨 수업자료 에이전트를 매일 오전 8시 40분(Asia/Seoul)에 실행해줘.
오늘 날짜를 포함하는 로컬 주간학습안내 파일을 읽고, 설정한 학년·반의 시간표를 확인해줘.
체육·영어를 제외한 과목의 인디스쿨 자료를 검색해 과목·검색어·제목·링크·핵심요약을
카카오톡 ‘나와의 채팅’으로 보내줘. 판독이나 로그인 세션이 불확실하면 추측하지 말고
확인 필요와 이유를 보내줘.
```
